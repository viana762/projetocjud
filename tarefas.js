...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!currentUser || currentUser.role !== 'ESTAGIARIO') {
    ...
    async function carregarMinhasTarefas() {
      try {
        const [tarefasResponse, notificacoesResponse] = await Promise.all([
          fetch(
            `https://projeto-cjud-backend.onrender.com/minhas-tarefas?internName=${encodeURIComponent(
              currentUserName
            )}`
          ),
          fetch(
            `https://projeto-cjud-backend.onrender.com/notifications?internName=${encodeURIComponent(
              currentUserName
            )}`
          ),
        ]);
    ...
    remindersContainer.addEventListener('click', async function (event) {
      ...
      if (closeButton) {
        ...
        if (notificationId) {
          try {
            await fetch(
              `https://projeto-cjud-backend.onrender.com/notifications/${notificationId}/read`,
              { method: 'PUT' }
            );
          } catch (error) {
